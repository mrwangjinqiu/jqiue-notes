...
+ `<label>`提供了内容绑定元素的能力，默认情况下点击内容是不会聚焦到元素的，只要将`for`属性和表单元素的`id`值关联起来就可以实现绑定，也可以将表单元素放入`<lable>`中实现，这样无需编写属性
...
```html
<audio controls src="http://s138.ananas.chaoxing.com/audio/ce/c1/78/769189bb72e811eb9d4f60ccf155cf62/audio.mp3">浏览器不支持</audio>
```
...
```html
<video controls src="/video.mp4" width="222px">浏览器不支持</video>
```
...
在以前的 Web 开发中，页面逻辑并不复杂，用户的要求也不是很好，因此`<table>`标签被大量地运用到页面布局上，而`<table>`本质上就是一个展示数据的元素，用来做布局元素导致页面逻辑不够清晰，所以`<div>`标签是用来解耦这种行为的，这样页面更加清晰，代码组织更加合理，利于前端开发的分工合作。于是产生了津津乐道的“div + css”这种页面布局模式，`<div>`就是用来做页面布局的容器元素的，本身没有任何语义
