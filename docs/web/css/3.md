...
+ `background-image`：设置背景图片，必须提供图片地址并放在`url()`中，如果图片的大小没有元素大，则会自动在水平和垂直方向平铺和填充背景
...
.foo {
  background-image: url(https://gitee.com/jqiue/img_upload/raw/master/images/avatar50.png);
}
.bar {
  background-image: url(http://p.ananas.chaoxing.com/star3/origin/522c96f46e7ef1f4c09607b6d9af7d44.png);
}
```
...
+ `background-position`：设置背景图片的在元素中的位置，默认情况下总是显示在元素的左上角，属性有两个取值，前面控制水平方向，后面控制垂直方向。像素可以精准的控制图片的位置，它的取值可以是负的
...
+ background-attachment：设置背景图片是否固定或者随着页面的其余部分滚动，`scroll`是默认属性随着页面的滚动而滚动，`fixed`则不会随着滚动
+ background-size:
...
.box{
  ...
  width: 100px;
  height: 100px;
  background: url(https://gitee.com/jqiue/img_upload/raw/master/images/spider.png) no-repeat;
}
.bar {
...
在 CSS3 中，通过转换属性能够对元素进行移动，缩放，旋转，拉长或拉伸
